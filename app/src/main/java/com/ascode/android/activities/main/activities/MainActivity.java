package com.ascode.android.activities.main.activities;
...
import com.ascode.android.activities.main.fragments.projects_store.ProjectsStoreFragment;
...
import com.ascode.android.lib.base.BottomSheetDialogView;
import com.ascode.android.metrics.StartupPerformanceTracker;
import com.ascode.android.utility.DataResetter;
                ...
                new SingleCopyTask(this, new SingleCopyTask.CallBackTask() {
                    ...
                    public void onCopyPostExecute(@NonNull String path, boolean wasSuccessful, @NonNull String reason) {
                        if (wasSuccessful) {
                            ...
                            AscodeUtil.toastError("Failed to copy backup file to temporary location: " + reason, Toast.LENGTH_LONG);
                        }
                    }
                }).copyFile(data);
            }
    ...
    private void allFilesAccessCheck() {
        if (Build.VERSION.SDK_INT > 29) {
            ...
            if (!optOutFile.exists() && !granted) {
                ...
                dialog.setMessage("Starting with Android 11, Android SCode needs a new permission to avoid " + "taking ages to build projects. Don't worry, we can't do more to storage than " + "with current granted permissions.");
