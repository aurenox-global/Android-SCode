package com.ascode.android.activities.resourceseditor.components.utils;
...
import com.ascode.android.utility.XmlUtil;
            ...
            if (attrId != 0 && referencingLimit > 0) {
                ...
                if (isNightVariant) {
                    if (material3LibraryManager.isDynamicColorsEnabled()) {
                        themedContext = new ContextThemeWrapper(context, R.style.ThemeOverlay_Ascode_ViewEditor_Material3_Dark);
                    } else {
                        themedContext = new ContextThemeWrapper(context, R.style.ThemeOverlay_Ascode_ViewEditor_Material3_NON_DYNAMIC_Dark);
                    }
                } else {
                    if (material3LibraryManager.isDynamicColorsEnabled()) {
                        themedContext = new ContextThemeWrapper(context, R.style.ThemeOverlay_Ascode_ViewEditor_Material3_Light);
                    } else {
                        themedContext = new ContextThemeWrapper(context, R.style.ThemeOverlay_Ascode_ViewEditor_Material3_NON_DYNAMIC_Light);
                    }
                }
                return String.format("#%06X", (0xFFFFFF & MaterialColors.getColor(themedContext, attrId, "getColorValue")));
            }
        } catch (Exception ignored) {
            android.util.Log.d("Ascode", "ColorsEditorManager: Exception ignored", ignored);
        }
        return defaultHexColor;
    }
        ...
        try {
            ...
            for (int i = 0; i < childNodes.getLength(); i++) {
                ...
                if (node.getNodeType() == Node.COMMENT_NODE) {
                    ...
                    if (PropertiesUtil.isHexColor(getColorValue(AscodeApplication.getContext(), colorValue, 4))) {
