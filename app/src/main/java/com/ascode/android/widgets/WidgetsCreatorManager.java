package com.ascode.android.widgets;
...
import com.ascode.android.databinding.WidgetsCreatorDialogBinding;
...
public class WidgetsCreatorManager {
    ...
    private void loadCustomWidgets() {
        try {
            ...
            AscodeUtil.toastError("Error loading widgets: " + e.getMessage());
            createWidgetsFile();
        }
    }
    ...
    private boolean isInvalidWidget(HashMap<String, Object> widgetData) {
        ...
        if (!widgetData.keySet().containsAll(requiredKeys)) {
            AscodeUtil.toastError("Missing required keys for widget: " + widgetData.get("title"));
            return true;
        }
        ...
        try {
            ...
            if (!availableWidgetsTypes.contains(String.valueOf(typeId))) {
                AscodeUtil.toastError("Invalid widget type: " + widgetData.get("title"));
                return true;
            }
        } catch (Exception e) {
            AscodeUtil.toastError("Invalid type format for widget: " + widgetData.get("title"));
            return true;
        }
    ...
    public void showWidgetsCreatorDialog(int position) {
        ...
        builder.setPositiveButton(Helper.getResString(R.string.common_word_save), (v, which) -> {
            try {
                ...
                AscodeUtil.toastError("Failed: " + e.getMessage());
            }
        });
    ...
    private void showMorePopUp(DialogInterface dialog, View anchorView) {
        ...
        popupMenu.setOnMenuItemClickListener(menuItem -> {
            ...
            if (menuItem.getItemId() == R.id.import_widgets) {
                ...
                AscodeUtil.toast("Exported in " + exportFilePath);
            }
            return true;
        });
        ...
        for (int i = 0; i < paths.length; i++) {
            ...
            try {
                ...
                if (importedWidgets.isEmpty()) {
                    AscodeUtil.toastError(String.format(ERROR_MESSAGE, i + 1));
                    continue;
                }
    ...
    public void addWidgetsByTitle(String title) {
        for (HashMap<String, Object> map : widgetConfigurationsList) {
            try {
                ...
                android.util.Log.d("Ascode", "WidgetsCreatorManager: Exception ignored", ignored);
            }
        }
    }
