...
public final class ProjectCodeInjector {
    ...
    private static long nextBlockId(ArrayList<BlockBean> blocks) {
        ...
        for (BlockBean block : blocks) {
            ...
            try {
                ...
                android.util.Log.d("Ascode", "ProjectCodeInjector: NumberFormatException ignored", ignored);
            }
        }
        return max + 1L;
    }
    ...
    private static void collectFiles(java.io.File root,
        ...
        if (name.endsWith(".java") || name.endsWith(".xml")) {
            ...
            out.add("• " + relative + " (" + com.ascode.android.utility.FileUtil.formatFileSize(current.length()) + ")");
            count[0]++;
        }
    }
}
